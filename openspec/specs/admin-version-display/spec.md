...
Defines a small, non-interactive version badge in the authenticated admin UI that surfaces the running server's version (sourced from `GET /api/health`), giving admins a quick way to confirm which build is deployed without cluttering the mobile layout or the login page.
