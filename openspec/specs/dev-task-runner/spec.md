...
Defines the Task-based (`go-task`) developer entrypoints — root and module-level Taskfiles — for building, running, and testing the server, agent, and web app locally, giving developers a single discoverable interface instead of remembering separate `go`/`npm` commands across modules.
