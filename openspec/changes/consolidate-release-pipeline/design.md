...
- Collapse each package's release-prep PR and infra-manifest-bump PR into one PR: version bump + CHANGELOG.md, in a single diff (the `deployment.yaml` image tag is deliberately *not* in this PR — see the ArgoCD correction below).
...
3. On every push to `main` matching the package's path filter, the workflow's first step checks `github.event.head_commit.message` for the substring `release-prep/gopints-v` (or `web-v`):
   ...
   - **No match** → this is an ordinary code push. Run GitVersion to compute the next version; if it's unchanged from the current highest matching tag (no releasable commits), stop — don't open an empty PR. Otherwise run git-cliff to generate the changelog entry and, for `web`, bump `package.json`'s version field, and open/update the consolidated PR via `peter-evans/create-pull-request`. (The `infrastructure/deploy/deployment.yaml` image tag is intentionally not touched here — see the ArgoCD correction below.)
...
- **[Risk — materialized, fixed]** The proposal originally accepted "the manifest references a version that isn't in GHCR yet" as a tolerable window, reasoning no live-cluster apply happens automatically. That assumption was wrong: this repo has ArgoCD with auto-sync watching it, so that window is a real `ImagePullBackOff` risk, not a theoretical one. → **Resolution**: moved the `deployment.yaml` image-tag bump out of the pre-merge release-prep PR entirely. It's now a direct commit to `main`, made by a new `bump-infra-version` job in `release.yml`, gated on `build-server`/`build-web` having actually succeeded — so the manifest is only ever updated once the image is confirmed to exist in GHCR. Still no extra PR for a human to review; the release-prep PR now only contains the version bump and changelog.
